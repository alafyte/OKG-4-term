...
// Вычисляет векторное произведение векторов V1 и V2
{
	...
	if (!b)
	{
		TCHAR* error = L"VectorMult: неправильные размерности векторов! ";
...
// Вычисляет модуль вектора V
{
	...
	if (!b)
	{
		TCHAR* error = L"ModVec: неправильнfz размерность вектора! ";
		MessageBox(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
...
// Вычисляет угол между векторами V1 и V2 в градусах
{
	...
	if (!b)
	{
		TCHAR* error = L"AngleV1V2: модуль одного или обоих векторов < 1e-7!";
		MessageBox(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	int b1 = (V1.cols() == 1) && (V1.rows() == 3);
	...
	if (!b)
	{
		TCHAR* error = L"AngleV1V2: неправильные размерности векторов! ";
		MessageBox(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	double cos_f = ScalarMult(V1, V2) / (modV1 * modV2);
	if (fabs(cos_f) > 1)
	{
		TCHAR* error = L"AngleV1V2: модуль cos(f)>1! ";
		MessageBox(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	double f;
