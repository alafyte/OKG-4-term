...
void CMainWnd::OnCPlot3DDef()
{
	...
	Graph1.SetFunction(Surface1, SpaceRect, dx, dy);	//задаем функцию
...
void CMainWnd::OnCplot3dFunc1()
{
	...
	GetClientRect(WinRect);              //получаем рабочую область
	WinRect.SetRect(WinRect.left + 50, WinRect.top + 50, WinRect.right - 50, WinRect.bottom - 50); //задаем рабочую область
	Graph2.SetFunction(Surface2, SpaceRect, dx, dy); //задаем функцию
...
void CMainWnd::OnCplot3dFunc2()
{
	...
	Graph3.SetFunction(Surface3, SpaceRect, dx, dy);
...
CMainWnd::CMainWnd()
{
	Create(NULL, L"Lab05", (WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX) &~(WS_THICKFRAME), rectDefault, NULL, NULL);
}
